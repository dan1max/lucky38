    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.4rem' }}>
      {selectable && (
        <div style={{
          ...
          color: held ? 'var(--gold-bright)' : 'transparent',
          fontWeight: 'bold',
        }}>
          HOLD
        </div>
          ...
          <div style={{ display: 'grid',
            ...
            {PAYTABLE.map((row) => (
              <div key={row.name} style={{
                ...
                background: result?.handName === row.name
                  ? 'rgba(201,168,76,0.15)' : 'transparent',
                border: result?.handName === row.name
                  ? '1px solid var(--gold)' : '1px solid transparent',
                borderRadius: '2px',
              }}>
                <span style={{ color: result?.handName === row.name
                  ? 'var(--gold)' : 'var(--white-dim)',
